...
spec = describe "HaskellWorks.Data.SuccinctSpec" $ do
  it "rank1 for BitShown (Vector Word8) and BitShown (Vector Word64) should give same answer" $ property $
  ...
  it "rank1 for BitShown (Vector Word16) and BitShown (Vector Word64) should give same answer" $ property $
    \(Count_0_64 i) (a :: Word16) (b :: Word16) (c :: Word16) (d :: Word16) ->
      ...
      rank1 vec16 i == rank1 vec64 i
  it "rank1 for BitShown (Vector Word32) and BitShown (Vector Word64) should give same answer" $ property $
