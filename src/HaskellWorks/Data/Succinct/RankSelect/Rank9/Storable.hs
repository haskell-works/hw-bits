...
-}
unsafeIndex :: StorableBitVector -> Position -> Bool
unsafeIndex i n = Bits.testBit w8 r
  where
    (q, r) = quotRem (getPosition n) 64
    w8 = Unboxed.unsafeIndex (bits i) q

{-| A `StorableBitVector` contains the original input vector and you can use
...
-}
index :: StorableBitVector -> Position -> Maybe Bool
index i n =
    if 0 <= n && n < size i
...
{-| A bit vector enriched with an index that adds O(1) `rank` and `select`
    ...
    The `StorableBitVector` increases the original bit vector's size by 87.5%
-}
data StorableBitVector = StorableBitVector
...
instance NFData StorableBitVector where
    rnf i = i `seq` ()

instance Arbitrary StorableBitVector where
...
{-|
-}
class SuccinctStorableBitVector v where
...
-}
rank :: SuccinctStorableBitVector a => a -> Position -> Maybe Count
rank sbv p0 = do
...
-}
select :: SuccinctStorableBitVector a => a -> Count -> Maybe Position
select sbv c0 = do
    n <- rank sbv (size sbv)
    guard (0 <= c0 && c0 < n)
...
instance SuccinctStorableBitVector Word64 where
...
instance SuccinctStorableBitVector BasicBlock where
...
instance SuccinctStorableBitVector IntermediateBlock where
...
instance SuccinctStorableBitVector SuperBlock where
...
each512thOneIndices :: Int -> Int -> Unboxed.Vector Word64 -> Unboxed.Vector Int
each512thOneIndices i1 i2 vector = Unboxed.unfoldr step (0 :: Int, i1)
    where
      step (n, offset) = case offset of
        x | x < i2 ->
          let wordOffset  = offset `Bits.shiftR` 6        in
          let bitOffset   = offset .&. 0x3f               in
          let word        = vector Unboxed.! wordOffset   in
          if Bits.testBit word bitOffset
            then if n `mod` 512 == 0
              then Just (offset, (n + 1, offset + 1))
              else step (n + 1, offset + 1)
            else step (n, offset + 1)
        x | x == i2 -> Just (i2 * 64, (n + 1, offset + 1))
        _           -> Nothing

oneIndices :: Int -> Int -> Unboxed.Vector Word64 -> Unboxed.Vector Int
oneIndices i1 i2 vector = Unboxed.unfoldr step i1
    where step offset = if offset < i2
            then  let wordOffset  = offset `Bits.shiftR` 6        in
                  let bitOffset   = offset .&. 0x3f               in
                  let word        = vector Unboxed.! wordOffset   in
                  if Bits.testBit word bitOffset
                    then Just (offset, offset + 1)
                    else step (offset + 1)
            else Nothing
{-# INLINE oneIndices #-}


{-| Create an `StorableBitVector` from a `Unboxed.Vector` of bits packed as `Word64`s

    You are responsible for padding your data to the next `Word64` boundary
-}
prepare :: Unboxed.Vector Word64 -> StorableBitVector
prepare v = StorableBitVector
    { rank9   = vRank
  ...
  where
    ...
    v1 = each512thOneIndices 0 (Unboxed.length v) v
                  ...
                  _ | numBasicBlocks < 1 ->
                    ...
                    | otherwise ->
                        ...
                        in  Unboxed.generate s (\i ->
                                ...
                                in  w64 ) )
          [0 .. Unboxed.length v1 - 2]))

instance SuccinctStorableBitVector StorableBitVector where
    partialRank (StorableBitVector rank9_ _ bits_) p0 = (c1 + c2 + c3, p3)
      where
    ...
    partialSelect (StorableBitVector rank9_ (Select9 primary_ secondary_) bits_) r =
