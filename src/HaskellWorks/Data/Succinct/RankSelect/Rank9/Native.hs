...
-}
unsafeIndex :: NativeBitVector -> Position -> Bool
...
-}
index :: NativeBitVector -> Position -> Maybe Bool
index i n =
    if 0 <= n && n < size i
...
{-| A bit vector enriched with an index that adds O(1) `rank` and `select`
    ...
    The `NativeBitVector` increases the original bit vector's size by 87.5%
-}
data NativeBitVector = NativeBitVector
...
instance NFData NativeBitVector where
    rnf i = i `seq` ()

instance Arbitrary NativeBitVector where
...
{-|
-}
class SuccinctNativeBitVector v where
...
-}
rank :: SuccinctNativeBitVector a => a -> Position -> Maybe Count
rank sbv p0 = do
...
-}
select :: SuccinctNativeBitVector a => a -> Count -> Maybe Position
select sbv c0 = do
    n <- rank sbv (size sbv)
    guard (0 <= c0 && c0 < n)
...
instance SuccinctNativeBitVector Word64 where
...
instance SuccinctNativeBitVector BasicBlock where
...
instance SuccinctNativeBitVector IntermediateBlock where
...
instance SuccinctNativeBitVector SuperBlock where
...
{-| Create an `NativeBitVector` from a `Unboxed.Vector` of bits packed as `Word64`s

    You are responsible for padding your data to the next `Word64` boundary
-}
prepare :: Unboxed.Vector Word64 -> NativeBitVector
prepare v = NativeBitVector
    { rank9   = vRank
...
instance SuccinctNativeBitVector NativeBitVector where
    partialRank (NativeBitVector rank9_ _ bits_) p0 = (c1 + c2 + c3, p3)
      where
